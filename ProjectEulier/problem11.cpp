...
int getMaxFromLines(int y, int x);
void problem11(){
...
//    }
    ...
    for(int y=0; y < 20; y++){
       for(int x=0; x < 20; x++){
           //printf("%d ", grid[y][x]);
           int maxFromLines = getMaxFromLines(y, x);
           max = getMaxFromPair(max, maxFromLines);
       }
       //printf("\n");
    }
...
//Gets the max product of 4 consecutive numbers either diagonally, vertically or horizontally.
int getMaxFromLines(int y, int x){
    int max;
